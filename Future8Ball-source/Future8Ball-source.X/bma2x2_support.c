...
s16 accel_x_s16_bak, accel_y_s16_bak, accel_z_s16_bak;

/* This function is an example for reading sensor data
 *	\param: None
 *	\return: communication result
 */
uint8_t bma2x2_check_motion(void)
{
...
*---------------------------------------------------------------------*/
	...
	if(abs(accel_x_s16_bak-accel_x_s16)>MOTION_THRESHOLD)
		{
		accel_x_s16_bak=accel_x_s16;
		motion=1;
		}
	if(abs(accel_y_s16_bak-accel_y_s16)>MOTION_THRESHOLD)
		{
		accel_y_s16_bak=accel_y_s16;
		motion=1;
		}
	if(abs(accel_z_s16_bak-accel_z_s16)>MOTION_THRESHOLD)
		{
		accel_z_s16_bak=accel_z_s16;
		motion=1;
		}
...
uint8_t bma2x2_check_orient(void)
{
	uint8_t orient=0;
	/* status of communication*/
	s32 com_rslt = ERROR;
	s16	accel_x_s16, accel_y_s16;
	/* Read the accel X data*/
	com_rslt += bma2x2_read_accel_x(&accel_x_s16);
	/* Read the accel Y data*/
	com_rslt += bma2x2_read_accel_y(&accel_y_s16);

	// |x|<200 and |y| <200, it is horizontal
	if(((-200<accel_y_s16)&&(accel_y_s16<200))&&((-200<accel_x_s16)&&(accel_x_s16<200)))
		{
		orient=4;			// hori
		return orient;
		}
	// |x|<200
	if((-200<accel_x_s16)&&(accel_x_s16<200))
		{
		if(accel_y_s16>200)
			{
			orient=1;			//up
	...
	// |y|<200
	if((-200<accel_y_s16)&&(accel_y_s16<200))
		{
		if(accel_x_s16>200)
			{
			orient=2;			//right
	...
	if(accel_x_s16<-200)
		{
		if(accel_y_s16<accel_x_s16)
			{
			orient=0;			//down
			return orient;
			}
		else if((accel_y_s16>200)&&(accel_y_s16>(-accel_x_s16)))
			{
			orient=1;			//up
			return orient;
			}
...
s8 BMA2x2_I2C_bus_write(u8 dev_addr, u8 reg_addr, u8 *reg_data, u8 cnt)
{
	...
	*/
		I2C2_MasterWrite(array, cnt+1, dev_addr, &status);  // pointer la valoarea wr_data
...
s8 BMA2x2_I2C_bus_read(u8 dev_addr, u8 reg_addr, u8 *reg_data, u8 cnt)
{
     ...
     * and FAILURE defined as -1
	 */
		uint8_t reg_address=reg_addr;
		// Checks if there is any more space in TRB queue
		while(I2C2_MasterQueueIsFull() == true);	// blocking
		I2C2_MasterWriteTRBBuild(&trb[0], &reg_address, 1, dev_addr);
		// Checks if there is any more space in TRB queue
		while(I2C2_MasterQueueIsFull() == true);	// blocking    
		//read 2 bytes for temperature
		I2C2_MasterReadTRBBuild(&trb[1], reg_data, cnt, dev_addr);
